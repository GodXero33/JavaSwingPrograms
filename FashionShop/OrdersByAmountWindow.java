...
public class OrdersByAmountWindow extends ChildWindow {
	...
	OrdersByAmountWindow () {
		...
		this.setWindowName("Orders By Amount");
		this.buildLayout();
	...
	private void sortRecordsByAmount () {
		...
		for (int i = 0; i < len; i++) {
			for (int j = 0; j < len - i - 1; j++) {
				if (OrderRecord.getAmount(this.records[j]) < OrderRecord.getAmount(this.records[j + 1])) {
					OrderRecord tmp = this.records[j];
	...
	private void addCustomEvents () {
		OrdersByAmountWindow thisInstance = this;
