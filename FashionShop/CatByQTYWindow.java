public class CatByQTYWindow extends ChildWindow {
	...
	CatByQTYWindow () {
		...
		this.setWindowName("Items By Qty");
		this.buildLayout();
		this.pack();
	}
	...
	private void sortRecordsByQTY () {
		...
		for (int i = 0; i < len; i++) {
			for (int j = 0; j < len - i - 1; j++) {
				if (ItemDetailRecord.getQty(this.records[j]) < ItemDetailRecord.getQty(this.records[j + 1])) {
