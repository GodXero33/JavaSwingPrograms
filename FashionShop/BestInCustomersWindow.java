...
public class BestInCustomersWindow extends ChildWindow {
	...
	BestInCustomersWindow () {
		...
		this.sortRecordsByAmount();
		this.setWindowName("Best In Customers");
		this.buildLayout();
		this.addCustomEvents();
		this.pack();
	}

	private void sortRecordsByAmount () {
		int len = this.records.length;

		for (int i = 0; i < len; i++) {
			for (int j = 0; j < len - i - 1; j++) {
				if (TotalOrderRecord.getAmount(this.records[j]) < TotalOrderRecord.getAmount(this.records[j + 1])) {
					TotalOrderRecord tmp = this.records[j];
	...
	private void addCustomEvents () {
		BestInCustomersWindow thisInstance = this;
